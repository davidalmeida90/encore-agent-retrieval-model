...
def loader_cache_enabled() -> bool:
    ...
    Returns:
        ...
        cache off.
    """
    # de-vendored: their config accessor is not available standalone, so the
    # on-disk loader cache is opt-in via env instead. Default off.
    return os.getenv("LOADER_DATA_CACHE", "").strip().lower() in ("1", "true", "yes")
...
def loader_cache_root() -> Path:
    ...
    """
    # de-vendored: cache root from env, see loader_cache_enabled above.
    root = os.getenv("LOADER_DATA_CACHE_ROOT") or ""
