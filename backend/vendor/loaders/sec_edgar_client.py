...
:mod:`vendor.loaders._http` throttle under the ``"sec"`` host bucket, and the
...
from vendor.loaders._http import (
    DEFAULT_USER_AGENT,
...
def _user_agent() -> str:
    ...
    # de-vendored: read the UA straight from the environment instead of
    # Vibe-Trading's config accessor. SEC requires a real contact address.
    override = os.getenv("SEC_EDGAR_UA") or os.getenv("SEC_EDGAR_EMAIL")
